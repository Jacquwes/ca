...
public interface AdministrationInterface {
  ...
  /**
   * Creates a new artist. There must not already be an artist with the same
   * lastName and firstName.
   *
   * @param lastName    the last name of the artist (non-empty string)
   * @param firstName   the first name of the artist (empty string "" if the
   *                    artist only has a last name and no first name)
   * @param nationality the nationality of the artist
   * @return the created artist or <code>null</code> in case of error (invalid
   *         parameters or an artist with the same values already exists)
   */
  Artist createArtist(String lastName, String firstName, String nationality);

  /**
   * Deletes an artist from the set of artists. Cannot be done if the artist is
   * associated with at least one movie as an actor or director.
   *
   ...
   * @return the created movie or <code>null</code> in case of problem (a movie
   *         with the same title already exists or the parameters were not valid)
   */
   ...
   * @param actors the list of actors to add. If actors from the list are already
   *               associated with the movie, they are not added again.
   * @return <code>true</code> if at least one actor from the list was added to
   *         the movie's actors, <code>false</code> otherwise
   */
   ...
   *
   * @param movie  the movie to modify
   * @param genres the list of genres to add. If genres from the list are already
   *               associated with the movie, they are not added again.
   * @return <code>true</code> if at least one genre from the list was added to
   *         the movie's genres, <code>false</code> otherwise
   */
  boolean addGenres(Movie movie, Genre... genres);

  /**
   * Adds a poster to a movie. If the movie already had a poster, it is replaced
   * by the new one.
   *
   * @param movie the movie to which to add a poster
   * @param file  the path of the file containing the poster image (in JPG, PNG
   *              format, etc.)
   * @return <code>true</code> if the poster was modified (the format and size
   *         were valid)
   ...
   * @return <code>true</code> if the movie was deleted or <code>false</code> in
   *         case of problem (the movie did not exist or the parameter was
   *         <code>null</code>)
   */
  ...
  /**
   * Opens a movie for rental. Does nothing if the movie was already open for
   * rental.
   *
   * @param movie the movie to open for rental
   * @return <code>true</code> if the movie is open for rental, <code>false</code>
   *         in case of problem (the movie was not found or value
   *         <code>null</code>)
   */
  boolean openRental(Movie movie);

  /**
   * Closes the rental of a movie. Does nothing if the movie was not open for
   * rental.
   *
   * @param movie the movie to close for rental
   * @return <code>true</code> if the movie is closed for rental,
   *         <code>false</code> in case of problem (the movie was not found or
